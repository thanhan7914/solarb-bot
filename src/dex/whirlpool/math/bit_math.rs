...
pub fn checked_mul_div_round_up_if(
    ...
    let p = n0.checked_mul(n1).ok_or(anyhow!("MulDivOverflow"))?;
    let n = p / d;
...
pub fn checked_mul_shift_right_round_up_if(
    ...
    let p = n0
        .checked_mul(n1)
        .ok_or(anyhow!("MultiplicationShiftRightOverflow"))?;

    let result = (p >> Q64_RESOLUTION) as u64;

    let should_round = round_up && (p & Q64_MASK > 0);
    if should_round && result == u64::MAX {
        return Err(anyhow!("MultiplicationOverflow"));
    }

    Ok(if should_round { result + 1 } else { result })
}

pub fn div_round_up(n: u128, d: u128) -> Result<u128> {
    div_round_up_if(n, d, true)
}

pub fn div_round_up_if(n: u128, d: u128, round_up: bool) -> Result<u128> {
    if d == 0 {
        return Err(anyhow!("DivideByZero"));
    }
